...
describe("TokenBankコントラクト", function () {
  ...
  beforeEach(async function () {
    ...
    await (await memberNFT.nftMint(owner.address, tokenURI1)).wait();
    await (await memberNFT.nftMint(addr1.address, tokenURI2)).wait();
    await (await memberNFT.nftMint(addr1.address, tokenURI3)).wait();
    await (await memberNFT.nftMint(addr2.address, tokenURI4)).wait();
  ...
  describe("Bankトランザクション", function () {
    ...
    it("トークン引き出しが実行できるべき", async function () {
      ...
      await (await tokenBank.connect(addr1).withdraw(100)).wait();
    ...
    it("預入トークンが不足していた場合、引き出しに失敗すべき", async function () {
      await expect(tokenBank.connect(addr1).withdraw(101)).to.be.revertedWith("An amount greater than your tokenBank balance!");
    });
    it("引き出し後には'TokenWithdraw'イベントが発行されるべき", async function () {
      await expect(tokenBank.connect(addr1).withdraw(100)).emit(tokenBank, "TokenWithdraw").withArgs(addr1.address, 100);
    });
    it("オーナーによる預入は失敗すべき", async function () {
      await expect(tokenBank.deposit(1)).to.be.revertedWith("Owner cannot execute");
    });
    it("オーナーによる引き出しは失敗すべき", async function () {
      await expect(tokenBank.withdraw(1)).to.be.revertedWith("Owner cannot execute");
    });
    it("トータル預入トークン数より大きな数はオーナーであっても移転に失敗すべき", async function () {
      await expect(tokenBank.transfer(addr1.address, 201)).to.be.revertedWith("Amounts greater than the total supply cannot be transferred");
    });
    it("NFTメンバー以外の移転は失敗すべき", async function () {
      await expect(tokenBank.connect(addr3).transfer(addr1.address, 1)).to.be.revertedWith("not NFT member");
    });
    it("NFTメンバー以外の預入は失敗すべき", async function () {
      await expect(tokenBank.connect(addr3).deposit(1)).to.be.revertedWith("not NFT member");
    });
    it("NFTメンバー以外の引き出しは失敗すべき", async function () {
      await expect(tokenBank.connect(addr3).withdraw(1)).to.be.revertedWith("not NFT member");
    });
  });
});