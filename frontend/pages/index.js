  ...
  const checkNft = async (addr) => {
    ...
    if (balance.toNumber() > 0) {
      ...
      for (let i = 0; i < balance.toNumber(); i++) {
        const tokenId = (await memberNFTContract.tokenOfOwnerByIndex(addr, i)).toNumber();
                          ...
                          <div key={i} className="flex justify-center pl-1 py-2 mb-1">
                            <div className="flex flex-col md:flex-row md:max-w-xl rounded-lg bg-white shadow-lg">
                              ...
                              <div className="p-6 flex flex-col justify-start">
                                ...
                                <p className="text-gray-600 text-xs">所有NFT# {item.tokenId}</p>
                              </div>
                            </div>
                          </div>
                        ))
